...
class ClinicA:
    ...
    def __init__(self,files: list):
        ...
        self.__files = files
        self.__data = None
        self.convert_data_columns = False
    ...
    async def read_excel_files(self):
        ...
        reading = [asyncio.to_thread(pd.read_excel,file) for file in self.__files]
        result = await asyncio.gather(*reading)

        self.__data = pd.concat(result,ignore_index=True)
    ...
    def _convert_dates(self):
        ...
        if not self.convert_data_columns:
            columns = ['warranty_until', 'install_date', 'last_calibration_date', 'last_service_date']
            for column in columns:
                self.__data[column] = pd.to_datetime(self.__data[column],format = 'mixed', dayfirst = True, errors='coerce')
            convert_data_columns = True

    async def filter_data_warranty(self):
        ...
        def sorted_data():
            ...
            self.__data['warranty_status'] = np.where(self.__data['warranty_until'] >= today,'Активна','Истекла')

            result = self.__data.sort_values(['warranty_status','warranty_until'] ,ascending = [True, True],na_position='last')

            return result

        return await asyncio.to_thread(sorted_data)

    async def clinics_most_problems(self):
        ...
        return await asyncio.to_thread(grouping)
    ...
    async def calibration_report(self):
        ...
        return await asyncio.to_thread(calibration)

    async def unification_information(self):
        ...
        return await asyncio.to_thread(normalize_data)

    async def write_excel(self,file):
        ...
        try:
            await self.read_excel_files()
            warranty_data = await self.filter_data_warranty()
            most_problems_information = await self.clinics_most_problems()
            calibration_data = await self.calibration_report()
            unification_information = await self.unification_information()

            def save_information():
                with pd.ExcelWriter(file, datetime_format='DD-MM-YYYY') as writer:
                    warranty_data.to_excel(writer, sheet_name="Sorted_data_warranty", index=False)
                    most_problems_information.to_excel(writer, sheet_name="Problems", index=False)
                    calibration_data.to_excel(writer, sheet_name = 'Calibration', index=False)
                    unification_information.to_excel(writer,sheet_name = 'All information', index=False)

            await asyncio.to_thread(save_information)

            print(f"Файл успешно сохранен: {file}")
...
if __name__ == "__main__":
    ...
    asyncio.run(clinic.write_excel("result_target.xlsx"))

    finish_time = time.time()
    all_time = finish_time - start_time

    print(f'Время выполнения асинхронного кода: {all_time}')